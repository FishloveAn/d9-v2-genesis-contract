...
| [inventory.json](inventory.json) | 143 provenance entries with explicit disposition references |
...
adds `d9-governance::VotingBodies`, classified `NotMigrated`: governance is
...
fills from `on_initialize`, so nothing is seeded or imported from V1. `HighestD9Price`
